...
function sort(books: LentBook[], column: SortColumn, direction: string): LentBook[] {
  if (direction === '' || column === '') {
    return books;
  } else {
...
function matches(book: LentBook, term: string, pipe: PipeTransform) {
  return book.id.toString().includes(term.toLowerCase())
    ...
    || book.firstName.toString().toLowerCase().includes(term.toLowerCase())
    || book.lastName.toString().toLowerCase().includes(term.toLowerCase())
    || book.dateLent.toString().toLowerCase().includes(term.toLowerCase())
    || book.comment.toString().toLowerCase().includes(term.toLowerCase());
}

@Injectable({
  providedIn: 'root',
})
export class LentBooksService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _books$ = new BehaviorSubject<LentBook[]>([]);
  private _total$ = new BehaviorSubject<number>(0);
  private books: LentBook[];
  ...
  public refresh(): void {
    this.bookDataService.getLentBooks().subscribe(result => {
      this.books = result;
      this._search$.pipe(
